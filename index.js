...
app.get("/", isAuthenticated, (req, res) => {
    const queries = {
        ...
        received:
            `SELECT SUM(quantity) as count FROM stock_transactions 
             WHERE transaction_type = 'receive' AND date(transaction_date) = date('now', 'localtime')`,
        dispatched:
            `SELECT SUM(quantity) as count FROM stock_transactions 
             WHERE transaction_type = 'dispatch' AND date(transaction_date) = date('now', 'localtime')`,
        lowStock: `SELECT COUNT(*) as count 
                   FROM stock WHERE warehouse_qty <= 5`,
